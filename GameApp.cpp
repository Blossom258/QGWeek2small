...
void GameApp::UpdateScene(float dt)
{
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D3) && m_CurrMode != ShowMode::Mixture)
	{
		

		m_CurrMode = ShowMode::Mixture;
		m_pd3dImmediateContext->IASetInputLayout(m_pVertexLayout3D.Get());
		auto meshData = Geometry::CreateBox();
		ResetMesh(meshData);
		m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pMixture.GetAddressOf());
	}
...
void GameApp::DrawScene()
{
	...
	if (m_CurrMode == ShowMode::WoodCrate)
	{
		...
		{
			if (i == 0)
				m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pOne.GetAddressOf());
			else if (i == 6)
				m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTwo.GetAddressOf());
			else if (i == 12)
				m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pThree.GetAddressOf());
			else if (i == 18)
				m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFour.GetAddressOf());
			else if (i == 24)
				m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFive.GetAddressOf());
			else if (i == 30)
				m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pSix.GetAddressOf());
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		m_pd2dRenderTarget->DrawTextW(textStr1.c_str(), (UINT32)textStr1.size(), m_pTextFormat.Get(),
			D2D1_RECT_F{ 475.0f,0.0f, 1000.0f, 200.0f }, m_pColorBrush.Get());
...
bool GameApp::InitResource()
{
	...
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\One.dds", nullptr, m_pOne.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\two.dds", nullptr, m_pTwo.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\three.dds", nullptr, m_pThree.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\four.dds", nullptr, m_pFour.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\five.dds", nullptr, m_pFive.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\six.dds", nullptr, m_pSix.GetAddressOf()));
	...
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\flareoutputB.dds", nullptr, m_pMixture.GetAddressOf()));
